...
function makeCompanyDbMethods({ connection }) {
  ...
  async function createCompany({ company_name, address, company_email,database_name }) {
    console.log("Create company");
    const result = await connection.query(
      `INSERT INTO ${database_name}.${table_name} (name, address,company_email) VALUES (?, ?,?);`,
      [company_name, address,company_email]
    );
  ...
  async function companyEmailByCompanyId({ company_id, database_name }) {
    console.log("companyEmail By CompanyID in data-access");
    console.log({company_id});
      const result = await connection.query(`SELECT company_email FROM ${database_name}.${table_name}
      WHERE company_id = ?`, [company_id]);
      console.log(result[0][0].company_email);
      return result[0][0].company_email;
  }
