console.log("employee data-access index.js");
...
function makeEmployeeDbMethods({ connection }) {
  ...
  async function createEmployee({
    ...
    console.log("Create employee data-access");
    {
      const result = await connection.query(
        `INSERT INTO ${database_name}.${table_name} (name,role,company_name,company_id) VALUES (?,?,?,?);`,
        [employee_name, role, company_name, company_id]
      );
  ...
  async function getEmployeesByCompanyName({ company_name, database_name }) {
    console.info("employee's of company data-access");
    const [result] = await connection.query(
